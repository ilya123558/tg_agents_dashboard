...
import type { EcoLead, EcoLeadStatus } from '@/entities/EcoLead';
import { useUpdateEcoLeadStatusMutation } from '@/entities/EcoLead';
...
const STATUS_OPTIONS: { value: EcoLeadStatus; label: string; color: string }[] = [
...
function EcoStatusSelect({ value, onChange }: { value: EcoLeadStatus; onChange: (v: EcoLeadStatus) => void }) {
...
export function EcoLeadCard({ lead, index = 0 }: { lead: EcoLead; index?: number }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -3 }}
      onClick={() => {
        if (typeof window !== 'undefined') sessionStorage.setItem('eco-scroll', String(window.scrollY));
        router.push(`/ecopulse/lead/${lead.id}`);
      }}
    ...
    >
      ...
      <div className="p-4 space-y-3">
        ...
        {/* ROW 7: thread context */}
        {lead.context && (
          <motion.div
            initial={{ opacity: 0, y: 6 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: index * 0.04 + 0.18, duration: 0.35, ease: [0.23, 1, 0.32, 1] }}
            className="bg-[#0a1410] border border-emerald-900/15 rounded-xl px-3 py-3"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-[10px] text-emerald-700/80 uppercase tracking-wider font-medium">Контекст</span>
            </div>
            <ThreadPreview context={lead.context} />
          </motion.div>
        )}

        {/* ROW 8: original text — collapsible */}
