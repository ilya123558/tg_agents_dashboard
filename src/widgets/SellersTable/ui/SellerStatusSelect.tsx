...
export function SellerStatusSelect({ value, onChange }: SellerStatusSelectProps) {
        ...
        <div className="absolute right-0 bottom-full mb-1 z-50 bg-[#1e1e1e] border border-white/10 rounded-xl shadow-xl overflow-hidden min-w-[130px]">
          {OPTIONS.map((opt) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 text-xs hover:bg-white/5 transition-colors flex items-center gap-2 ${opt.value === value ? 'bg-white/5' : ''}`}
            >
