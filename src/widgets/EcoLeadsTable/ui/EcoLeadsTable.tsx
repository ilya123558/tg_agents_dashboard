...
import type { EcoLead, EcoLeadStatus, EcoReadiness } from '@/entities/EcoLead';
import { EcoLeadCard } from '@/widgets/EcoLeadCard';
...
const STATUSES: (EcoLeadStatus | 'все')[] = ['все', 'новый', 'отправлено', 'ответил', 'не ответил', 'архив'];
const READINESS_FILTERS: (EcoReadiness | 'все')[] = ['все', 'горячий', 'тёплый', 'холодный'];

const READINESS_BADGE: Record<EcoReadiness, { label: string; active: string; inactive: string }> = {
...
function sortLeads(leads: EcoLead[], sort: SortKey): EcoLead[] {
...
interface Props { leads: EcoLead[] }

export function EcoLeadsTable({ leads }: Props) {
  const [statusFilter, setStatusFilter]       = useState<EcoLeadStatus | 'все'>('все');
  const [readinessFilter, setReadinessFilter] = useState<EcoReadiness | 'все'>('все');
  ...
  useEffect(() => {
    try {
      const saved = sessionStorage.getItem('eco-filters');
      if (saved) {
  ...
  useEffect(() => {
    if (!hydrated) return;
    const scroll = sessionStorage.getItem('eco-scroll');
    if (scroll) {
      sessionStorage.removeItem('eco-scroll');
  ...
  useEffect(() => {
    if (!hydrated) return;
    sessionStorage.setItem('eco-filters', JSON.stringify({ statusFilter, readinessFilter, genderFilter, sort }));
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {READINESS_FILTERS.filter((r) => r !== 'все').map((r) => {
            const cfg = READINESS_BADGE[r as EcoReadiness];
            return (
              <motion.button
                key={r}
                whileTap={{ scale: 0.92 }}
      ...
      {filtered.length === 0 ? (
        ...
        <AnimatePresence mode="popLayout">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
            {filtered.map((lead, i) => (
              <EcoLeadCard key={lead.id} lead={lead} index={i} />
            ))}
          </div>
        </AnimatePresence>
      )}
