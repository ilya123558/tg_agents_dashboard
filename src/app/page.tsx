...
export default function ChinaPage() {
  const { data, isLoading, isError, refetch } = useGetLeadsQuery();
    ...
    <>
      ...
      <header className="border-b border-white/5 px-4 md:px-6 py-4 flex items-center justify-between sticky top-0 bg-[#0f0f0f]/90 backdrop-blur-sm z-10">
        <div className="flex items-center gap-2">
          <span className="rounded overflow-hidden"><ChinaFlag size={22} /></span>
          <span className="font-medium text-white text-sm">Китайские товары</span>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <main className="px-4 md:px-6 py-5 space-y-4 max-w-[1400px] mx-auto w-full">
        ...
        {isError && (
          <div className="text-center py-20 text-red-500 text-sm">
            Ошибка загрузки. Проверь NOTION_API_KEY в .env.local
          </div>
        )}

        {data && (
          <>
            <StatsPanel leads={data.leads} />
            <div className="flex flex-col lg:flex-row gap-4 items-start">
              <div className="flex-1 min-w-0 w-full">
                <LeadsTable leads={data.leads} />
              </div>
              <div className="w-full lg:w-72 lg:shrink-0 lg:sticky lg:top-20">
                <GroupsStats leads={data.leads} />
              </div>
            </div>
          </>
        )}
