...
import { useGetBioLeadsQuery, useUpdateBioLeadStatusMutation } from '@/entities/BioLead';
import type { BioLeadStatus } from '@/entities/BioLead';
...
const STATUS_OPTIONS: { value: BioLeadStatus; label: string; color: string; dot: string }[] = [
...
export default function BioLeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const router = useRouter();
  const { data, isLoading } = useGetBioLeadsQuery();
      ...
      <header className="sticky top-0 z-30 bg-[#080f0a]/95 backdrop-blur-sm">
        ...
        <div className="px-4 md:px-6 h-14 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <span className="text-[11px] text-gray-600 shrink-0 hidden sm:inline">Биохакинг</span>
