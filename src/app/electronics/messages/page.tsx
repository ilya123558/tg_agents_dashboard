...
export default function MessagesPage() {
  ...
  const { data, isLoading, isError } = useGetLeadsQuery(undefined, { pollingInterval: 60_000 });
  ...
  // Одноразовая чистка старых моков localStorage
  useEffect(() => {
    cleanupLegacyStorage();
  }, []);

  // Читаем ?chat= из URL при монтировании
  useEffect(() => {
  ...
  // На мобиле когда диалог открыт — скрываем глобальную нижнюю навигацию
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col bg-[#0f0f0f] overflow-hidden h-[100dvh] md:h-screen">
      {/* Header — на мобиле скрываем когда диалог открыт (его место занимает шапка чата) */}
      <header className={`${activeLead ? 'hidden md:flex' : 'flex'}
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="text-[11px] text-gray-600 truncate hidden sm:inline">· Электроника</span>
        </div>

        {/* Счётчик по менеджерам */}
